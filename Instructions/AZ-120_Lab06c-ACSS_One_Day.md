...
- Aufgabe 2: Konfigurieren von Rollenzuweisungen für die rollenbasierte Azure-Zugriffssteuerung (RBAC) für die vom Benutzer zugewiesene Microsoft Entra ID
...
>**Wichtig:** Die in dieser Übung implementierten Voraussetzungen sollen *nicht* die bewährten Methoden für die Bereitstellung von SAP-Workloads in Azure mithilfe von Azure Center für SAP-Lösungen darstellen. Ihr Zweck besteht darin, Zeit, Kosten und Ressourcen zu minimieren, die erforderlich sind, um die Mechanismen der Bereitstellung von SAP-Workloads in Azure mithilfe von Azure Center für SAP-Lösungen zu bewerten und Verwaltungs- und Wartungsaufgaben nach der Bereitstellung durchzuführen.
...
In dieser Aufgabe erstellen Sie eine von Microsoft Entra zugewiesene verwaltete Identität, die vom Azure Center für SAP-Lösungen für Azure Storage-Zugriff während der Bereitstellung verwendet werden soll.

1. Starten Sie auf dem Lab-Computer einen Webbrowser, navigieren Sie zum Azure-Portal unter `https://portal.azure.com` und authentifizieren Sie sich mithilfe eines Microsoft-Kontos oder eines Microsoft Entra-ID-Kontos mit der Rolle „Besitzer“ im Azure-Abonnement, das Sie in dieser Übung verwenden.
...
1. Geben Sie auf der Registerkarte **Grundlagen** der Seite **Vom Benutzer zugewiesene verwaltete Identität erstellen** die folgenden Einstellungen an und wählen Sie dann **Überprüfen + Erstellen** aus:
   ...
   |Resource group|Der Name einer **neuen** Ressourcengruppe **acss-infra-RG**|
...
1. Klicken Sie auf der Seite **Zugriffssteuerung (IAM)** auf **+ Hinzufügen** und im Dropdownmenü auf **Rollenzuweisung hinzufügen**.
1. Suchen Sie auf der Registerkarte **Rolle** der Seite **Rollenzuweisung hinzufügen** in der Auflistung der Rollen **Auftragsfunktion** nach dem Eintrag **Azure Center für SAP-Lösungen-Dienstrolle** und wählen Sie **Weiter** aus.
...
1. Suchen und wählen Sie auf der Registerkarte **Rolle** der Seite **Rollenzuweisung hinzufügen** in der Liste der Rollen **Auftragsfunktionsrollen** aus, suchen Sie nach dem **Azure Center für SAP-Lösungenadministrator**-Eintrag, und wählen Sie **Weiter** aus.
1. Auf der Registerkarte **Mitglieder**
   - für **Zugriff zuweisen auf**, wählen Sie **Benutzer-, Gruppen- oder Dienstprinzip** aus,
...
1. Zurück auf dem Blatt **Zugriffssteuerung (IAM)** wählen Sie **+ Hinzufügen** aus und im Dropdownmenü klicken Sie auf **Rollenzuweisung hinzufügen**.
1. Suchen Sie auf der Registerkarte **Rolle** der Seite **Rollenzuweisung hinzufügen** in der Auflistung **Auftragsfunktionsrollen** nach dem Eintrag **Verwalteter Identitätsoperator** und wählen Sie **Weiter** aus.
1. Auf der Registerkarte **Mitglieder**
   - für **Zugriff zuweisen auf**, wählen Sie **Benutzer-, Gruppen- oder Dienstprinzip** aus,
   - klicken Sie auf **+Mitglieder auswählen**.
1. Geben Sie im Bereich **Mitglieder auswählen** im Textfeld **Auswählen** den Namen des Microsoft Entra ID-Benutzerkontos ein, das Sie für den Zugriff auf das Azure-Abonnement verwendet haben, das Sie für diese Übung verwenden, wählen Sie es in der Liste der Ergebnisse aus, die Ihrem Eintrag entsprechen, und klicken Sie dann auf **Auswählen**.
...
1. Suchen Sie auf dem Lab-Computer im Webbrowserfenster, in dem das Azure-Portal angezeigt wird, im Textfeld **Suchen** nach **virtuellen Netzwerken** und wählen Sie sie aus.
...
1. Geben Sie im Bereich **Subnetz** die folgenden Einstellungen an und wählen Sie dann **Hinzufügen** aus (belassen Sie andere Personen bei ihren Standardwerten):
...
1. Wählen Sie auf der Registerkarte **IP-Adressen** **Überprüfen + erstellen** aus:
...
- Verwenden Sie Azure Center für SAP-Lösungen, um die Infrastruktur bereitzustellen, die SAP-Workloads in einem Azure-Abonnement hosten kann.

Diese Aktivität entspricht der folgenden Aufgabe dieser Übung:

- Aufgabe 1: Erstellen einer virtuellen Instanz für SAP-Lösungen (VIS)

>**Hinweis:** Nach der erfolgreichen Bereitstellung können Sie mit der Installation von SAP-Software fortfahren, indem Sie Azure Center für SAP-Lösungen verwenden. Die Installation von SAP-Software ist jedoch in diesem Lab nicht enthalten.

>**Hinweis:** Informationen zum Installieren von SAP-Software mithilfe von Azure Center für SAP-Lösungen finden Sie in der Microsoft Learn-Dokumentation, in der beschrieben wird, wie Sie [SAP-Installationsmedien abrufen](https://learn.microsoft.com/en-us/azure/sap/center-sap-solutions/get-sap-installation-media) und [SAP-Software installieren](https://learn.microsoft.com/en-us/azure/sap/center-sap-solutions/install-software).
...
1. Geben Sie auf der Registerkarte **Grundlagen** der Seite **Erstellen einer virtuellen Instanz für SAP-Lösungen** die folgenden Einstellungen an, und wählen Sie **Weiter: Virtuelle Computer**
   ...
   |Imageoptionen für Datenbankbetriebssysteme|**Verwenden eines Marketplace-Images**|
...
1. Geben Sie auf der Registerkarte **virtuelle Computer** die folgenden Einstellungen an:
   ...
   |SAPS für die Anwendungsebene|**1000**|
...
>**Hinweis:** Wenn Sie Azure Backup auf der VIS-Ressourcenebene im Azure Center für SAP-Lösungen konfigurieren, können Sie in einem Schritt die Sicherung für Azure-VMs, welche die Datenbank, Anwendungsserver und SAP Central Services-Instanz hosten, und für die HANA DB aktivieren. Für die HANA DB-Sicherung führt Azure Center für SAP-Lösungen automatisch das Sicherungsvorregistrierungsskript aus.

1. Suchen Sie auf dem Lab-Computer im Microsoft Edge-Fenster, in dem das Azure-Portal angezeigt wird, im Textfeld **Suchen** nach **Azure Center für SAP-Lösungen**.
1. Wählen Sie auf der Seite **Azure Center für SAP-Lösungen \| Übersicht** im vertikalen Navigationsmenü auf der linken Seite **Virtuelle Instanzen für SAP-Lösungen** aus und wählen Sie in der Liste der virtuellen Instanzen die Instanz aus, die Sie in der vorherigen Übung bereitgestellt haben.
...
1. Beachten Sie die Meldung, dass die Sicherung nicht eingerichtet werden kann, da die SAP-Softwareinstallation/-registrierung für dieses SAP-System nicht abgeschlossen ist.

   >**Hinweis:** Dies entspricht dem erwarteten Verhalten. Sie können die Sicherung auf diese Weise erst einrichten, wenn die Installation der SAP-Software abgeschlossen ist. Das Abschließen des Setups umfasst jedoch auch zusätzliche Voraussetzungen, einschließlich der Erstellung von Tresoren und Sicherungsrichtlinien, die wir hier überprüfen werden. 

1. Suchen Sie auf dem Lab-Computer im Microsoft Edge-Fenster, in dem das Azure-Portal angezeigt wird, im Textfeld **Suchen** nach dem **Sicherungscenter** und wählen Sie es aus. 
1. Wählen Sie auf der Seite **Sicherungscenter** im vertikalen Navigationsmenü auf der linken Seite im Abschnitt **Verwalten** **Tresore** aus.
1. Wählen Sie auf der Seite **Sicherungscenter \| Tresore** **+ Tresor** aus.
1. Bei **Start: Tresor erstellen**, überprüfen Sie die verfügbaren Tresortypen, stellen Sie sicher, dass **Wiederherstellungsdienste** (die **virtuelle Azure-Computer** und **SAP HANA in Azure VM**-Datenquellentypen unterstützt) ausgewählt ist, und wählen Sie dann **Weiter** aus.
1. Geben Sie auf der Registerkarte **Grundlagen** der Seite **Tresor erstellen** die folgenden Einstellungen an und wählen Sie **Weiter: Redundanz**
...
1. Geben Sie auf der Registerkarte **Redundanz** die folgenden Einstellungen an und wählen Sie **Weiter: Tresoreigenschaften**
...
1. Überprüfen Sie auf der Registerkarte **Vault-Eigenschaften** die Einstellung **Unveränderlichkeit** aktivieren, ohne sie zu aktivieren, und wählen Sie **Weiter: Netzwerk**
...
>**Hinweis:** Während der Azure Center für SAP-Lösungsdienst ein zonenredundanter Dienst ist, gibt es kein von Microsoft initiiertes Failover im Falle eines Regionsausfalls. Um dieses Szenario zu nachzubessern, sollten Sie die Notfallwiederherstellung für SAP-Systeme konfigurieren, die mithilfe von Azure Center für SAP-Lösungen bereitgestellt wird, indem Sie die in der [Übersicht über die Notfallwiederherstellung beschriebenen Anleitungen und Infrastrukturrichtlinien für SAP-Workload](https://learn.microsoft.com/en-us/azure/sap/workloads/disaster-recovery-overview-guide)befolgen, welche die Verwendung von Azure Site Recovery (ASR) umfassen. In dieser Aufgabe durchlaufen Sie den Prozess der Implementierung einer ASR-basierten Notfallwiederherstellungslösung, die auf diesen Leitfaden basiert.
...
1. Suchen Sie auf dem Lab-Computer im Microsoft Edge-Fenster, in dem das Azure-Portal angezeigt wird, im Textfeld **Suchen** nach **Wiederherstellungsdiensten** und wählen Sie sie aus.
1. Wählen Sie im Menü **Recovery Services-Tresore** die Option **+ Hinzufügen**.
1. Geben Sie auf der Registerkarte **Grundlagen** der Seite **Erstellen eines Recovery Services-Tresor** die folgenden Einstellungen an (belassen Sie die anderen bei ihren Standardwerten), und wählen Sie **Weiter: Redundanz**.
   ...
   >**Hinweis:** Informationen zur Identifizierung der Region, die mit der Region gekoppelt ist, die Ihre Produktionsworkloads hostet, finden Sie in der MS Learn-Dokumentation zur Beschreibung von [mit Azure gekoppelten Regionen](https://learn.microsoft.com/en-us/azure/reliability/cross-region-replication-azure#azure-paired-regions).
...
1. Warten Sie auf der Registerkarte **Überprüfen und Erstellen** bis der Überprüfungsprozess abgeschlossen ist, und wählen Sie **Erstellen**aus.
   ...
   >**Hinweis:** Jetzt richten Sie die Notfallwiederherstellungsumgebung in der gekoppelten Region ein, in der Sie den Recovery Services-Tresor erstellt haben. Diese Umgebung enthält ein virtuelles Netzwerk, das Replikate der Azure-VMs hostet, die derzeit in der primären Region gehostet werden, in der Sie die virtuelle Instanz für SAP bereitgestellt haben. 

1. Suchen Sie auf dem Lab-Computer im Microsoft Edge-Fenster, in dem das Azure-Portal angezeigt wird, im Textfeld **Suchen** nach **Wiederherstellungsdiensten** und wählen Sie sie aus.
1. Wählen Sie auf der Seite mit den **Recovery Services-Tresoren** **acss-dr-RSV** aus.
...
1. Wählen Sie auf der Seite **acss-dr-RSV \| Site Recovery** im Abschnitt **Virtuelle Azure-Computer** **1 aus. Aktivieren der Replikation**. 
1. Legen Sie in der Registerkarte **Quelle** der Seite **Replikation aktivieren** die folgenden Einstellungen an und wählen Sie dann **Nächste** aus:

   |Einstellung|Wert|
   |---|---|
   |Region|den Namen der Azure-Region, in der Ihre virtuelle Instanz für SAP (VIS) gehostet wird|
   |Abonnement|Der Name des Azure-Abonnements, das in dieser Übung verwendet wird|
...
1. Wählen Sie auf der Registerkarte **virtuelle Computer** die ersten vier virtuellen Computer in der Liste (**vi1appvm0**, **vi1appvm1**, **vi1ascsvm0** und **vi1ascsvm0**) aus und wählen Sie dann **Weiter**.

   >**Hinweis:** Wie bereits erwähnt, wird die ASR-basierte Replikation auf die Anwendungsserver und SAP Central Services-Instanzen angewendet. Datenbankserver werden synchronisiert, indem sie auf die systemeigene Datenbankreplikationsfunktionalität vertrauen.

1. Führen Sie auf der Registerkarte **Replikationseinstellungen** die folgenden Aktionen aus:

   1. Wählen Sie bei Bedarf in der Dropdownliste **Zielspeicherort** die Azure-Region aus, in der Sie den **acss-dr-RSV**-Recovery Services-Tresor erstellt haben.
   1. Stellen Sie sicher, dass der Name des in dieser Übung verwendeten Azure-Abonnements in der Dropdownliste **Zielabonnement** angezeigt wird.
   1. Wählen Sie in der Dropdownliste **Zielressourcengruppe** **acss-dr-RG** aus.
   1. Wählen Sie unter der Dropdownliste **Virtuelles Failovernetzwerk** die Option **Neu erstellen** aus.
   1. Geben Sie im Bereich **Erstellen eines virtuellen Netzwerks** im Textfeld **Name** **CONTOSO-VNET-asr** ein
   1. Ersetzen Sie im Abschnitt **Adressraum** im Textfeld **Adressbereich** den Standardeintrag durch **10.10.0.0/16**.
   1. Geben Sie im Abschnitt **Subnetze** im Textfeld **Subnetznamen** **App** ein, und geben Sie im Textfeld **Adressbereich** **10.10.0.0/24** ein.
   1. Geben Sie unter dem neu hinzugefügten Subnetzeintrag im Abschnitt **Subnetze** im Textfeld **Subnetzname** **db** ein, und geben Sie im Textfeld **Adressbereich** **10.10.2.0/24** ein.
   1. Wählen Sie im Bereich **Erstellen eines virtuellen Netzwerks** **OK** aus.
   1. Stellen Sie wieder auf der Seite **Replikation** aktivieren sicher, dass der Eintrag **(neue) App (10.10.0.0/24)** in der Dropdownliste **Failover Subnetz** angezeigt wird.
   ...
   1. Auf der Seite **Verfügbarkeitsoptionen** haben Sie die Möglichkeit, Näherungsgruppen für Zielressourcen zu implementieren. Nehmen Sie aber keine Änderungen vor und wählen Sie **Abbrechen** aus.
...
1. Überprüfen Sie auf der Registerkarte **Überprüfen** auf der Seite **Replikation aktivieren** die Einstellungen. Für diese Übung wird die Replikation **nicht aktiviert**.
   ...
   >**Hinweis:** Sie können an diesem Punkt sowohl Azure Bastion als auch Azure Firewall bereitstellen, aber Sie sollten die Bereitstellung stattdessen im Rahmen des Verfahrens Ihres Notfallwiederherstellungsfailovers automatisieren. Dadurch werden Die Gebühren für die Aufrechterhaltung der Notfallwiederherstellungsumgebung minimiert. Das gleiche gilt für andere Komponenten dieser Umgebung, welche die Konfiguration der primären virtuellen Instanz für SAP widerspiegeln, z. B. Azure Premium-Dateifreigaben und benutzerdefiniertes Routing.

#### Aufgabe 3: Überprüfen der Überwachungsoptionen für SAP-Workloads, die vom Azure Center für SAP-Lösungen verwaltet werden

>**Hinweis:** Wie bei der Sicherung können Sie die Überwachungsfunktionen von Azure Center für SAP-Lösungen nicht vollständig erleben. Dies erfordert die Installation von SAP-Software oder das Registrieren eines vorhandenen Azure Monitor für SAP-Lösungsinstanzen. Stattdessen durchlaufen Sie in dieser Aufgabe die Schnittstelle, die in der Virtual Instance für SAP-Lösungen verfügbar ist, um diese Funktionen zu identifizieren und zu überprüfen.

1. Suchen Sie auf dem Lab-Computer im Microsoft Edge-Fenster, in dem das Azure-Portal angezeigt wird, im Textfeld **Suchen** nach **virtuellen Instanzen für SAP-Lösungen** und wählen Sie sie aus. 
1. Überprüfen Sie auf der Seite **Virtuelle Instanzen für SAP-Lösungen** die zusammengefassten Statusinformationen für die **VI1**-Instanz, einschließlich der visuellen Gesamtanzeigen für Integrität und Status.
1. Wählen Sie auf der Seite **Virtuelle Instanzen für SAP-Lösungen** **VI1** aus.
1. Wählen Sie auf der Seite **VI1** im vertikalen Navigationsmenü auf der linken Seite **Übersicht** aus und wählen Sie dann im Bereich auf der rechten Seite **Überwachung** aus.
1. Überprüfen Sie die im Überwachungsbereich angezeigte Überwachungstelemetrie.

   >**Hinweis:** Der Überwachungsbereich enthält vCPU-Auslastungsdiagramme und Metriken für Anwendungsserver, Datenbankserver und SAP Central Services-Instanzen. Sie enthält auch Datenbankdatenträger-IOPS-Statistikdatenbankserver. 

1. Wählen Sie auf der Seite **VI1** im vertikalen Navigationsmenü auf der linken Seite im Abschnitt **Überwachung** **Quality Insights** aus.
1. Überprüfen Sie auf der Seite **VI1 \| Quality Insights \| Arbeitsmappe 1** die Registerkarte **Advisor-Empfehlung**, die Empfehlungen für die Optimierung der Virtual Instance für SAP-Lösungen (VIS), der zentralen Serverinstanz, der App-Dienstinstanzen und Datenbanken enthält.

   >**Hinweis:** Für diese Empfehlungen ist die Installation der SAP-Software erforderlich.

1. Wählen Sie auf der Seite **VI1 \| Quality Insights \| Arbeitsmappe 1** die Registerkarte **VM** aus und überprüfen Sie den Inhalt der Registerkarten **Azure Compute**, **Compute List**, **Compute Extensions**, **Compute + OS Disk** und **Compute + Datenträger**.

   >**Hinweis:** Jede dieser Registerkarten sollte tatsächliche Daten enthalten, die aus den Azure-VMs gesammelt werden, die Teil der virtuellen Instanz für SAP-Lösungen sind.

1. Wählen Sie auf der Seite **VI1 \| Quality Insights \| Arbeitsmappe 1** die Registerkarte **Konfigurationsüberprüfungen** aus und überprüfen Sie den Inhalt der Registerkarten **Beschleunigtes Netzwerk**, **Öffentliche IP**, **Sicherung** und **Load Balancer**. Dieser Inhalt bietet einen schnellen Überblick über die Leistungs- und Sicherheitseinstellungen von Compute- und Netzwerkkomponenten der Virtual Instance für SAP-Lösungen. Die Registerkarte **Load Balancer** enthält **Load Balancer Monitor**-Informationen, die wichtige Lastenausgleichsmetriken anzeigen.
1. Wählen Sie auf der Seite **VI1** im vertikalen Navigationsmenü auf der linken Seite im Abschnitt **Überwachung** **Azure Monitor für SAP-Lösungen** aus.
1. Beachten Sie auf der Seite **VI1 \| Azure Monitor für SAP-Lösungen** die Meldung, dass AMS nicht eingerichtet werden kann, da die SAP-Softwareinstallation\Registrierung für VIS nicht abgeschlossen ist.

   >**Hinweis:** Nachdem Sie SAP-Software installiert haben, können Sie sie in eine neue oder vorhandene Azure Monitor-Ressource für SAP-Lösungen integrieren. Azure Monitor für SAP-Lösungen basiert auf den Azure Monitor-Funktionen von Log Analytics und Arbeitsmappen, um eine umfassende Überwachung von SAP-Workloads bereitzustellen, die auf Azure-VMs gehostet werden, einschließlich von Unterstützung für benutzerdefinierte Visualisierungen, Abfragen und Warnungen.
